...
/*   Updated: 2025/10/05 20:25:56 by jmagand          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_filename(t_data *data, char *input, char *dot)
{
	...
	if (!data->file->filename && input[0] != '.')
		...
		free_and_exit(data, msg_predefined(EMPTY_FILENAME), 0);
}
...
static void	check_input(char *input, t_data *data)
{
	...
	if (dot)
	{
		...
		if (!data->file->ext)
			free_and_exit(data, msg_predefined(EMPTY_EXT), 0);
		if (ft_strcmp(data->file->ext, "cub"))
			free_and_exit(data, msg_predefined(BAD_EXT), 0);
	}
	else
